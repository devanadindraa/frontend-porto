...
export function DarkModeToggle() {
  ...
  useEffect(() => {
    ...
    if (darkModeToggle.current) {
      darkModeToggle.current.addEventListener('change', handleChange);
    }

    // Cleanup event listener on unmount
    return () => {
      if (darkModeToggle.current) {
        darkModeToggle.current.removeEventListener('change', handleChange);
      }
    };
  }, []);

  return (
    <div className="flex dark-mode-toggle-container">
      <input
        type="checkbox"
        id="darkmode-toggle"
        ref={darkModeToggle}
        // Add a default checked attribute to ensure the checkbox reflects the dark mode state
